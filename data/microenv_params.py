...
class MicroenvTab(object):
    ...
    def __init__(self):
        ...
        menv_var1 = Button(description='signal', disabled=True, layout=name_button_layout)
        ...
        self.signal_diffusion_coefficient = FloatText(value=100000.0,
        ...
        self.signal_decay_rate = FloatText(value=10,
        ...
        row_signal = [menv_var1,  ] 
        row1 = [param_name1, self.signal_diffusion_coefficient, menv_units_button1]
        row2 = [param_name2, self.signal_decay_rate, menv_units_button2]
        row3 = [param_name3, self.signal_initial_condition, units_button1]
        row4 = [param_name4, self.signal_Dirichlet_boundary_condition, units_button2, self.signal_Dirichlet_boundary_condition_toggle]
    ...
    def fill_gui(self, xml_root):
        ...
        self.signal_diffusion_coefficient.value = float(vp[0].find('.//diffusion_coefficient').text)
        self.signal_decay_rate.value = float(vp[0].find('.//decay_rate').text)
        self.signal_initial_condition.value = float(vp[0].find('.//initial_condition').text)
        self.signal_Dirichlet_boundary_condition.value = float(vp[0].find('.//Dirichlet_boundary_condition').text)
    ...
    def fill_xml(self, xml_root):
        ...
        uep = xml_root.find('.//microenvironment_setup')  # find unique entry point
        vp[0].find('.//diffusion_coefficient').text = str(self.signal_diffusion_coefficient.value)
        vp[0].find('.//decay_rate').text = str(self.signal_decay_rate.value)
        vp[0].find('.//initial_condition').text = str(self.signal_initial_condition.value)
        vp[0].find('.//Dirichlet_boundary_condition').text = str(self.signal_Dirichlet_boundary_condition.value)
        vp[0].find('.//Dirichlet_boundary_condition').attrib['enabled'] = str(self.signal_Dirichlet_boundary_condition_toggle.value).lower()
